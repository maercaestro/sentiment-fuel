...
df['sales'] = (df['overall_score'] * df['projected_population'] * 40 * 54 * 0.6)/1000
df['new_score'] = (0.8*df['rating']) + (0.2 * (df['Sentiment_Score']+1))
df['new_sales'] = (df['new_score'] * df['projected_population'] * 40 * 54 * 0.6)/1000
...
with col1:
    ...
    if st.button('Get GPT-3.5 Business Recommendation'):
        ...
        st.write(f"Doing this will increase your potential sales from RM {selected_station_data['sales']} to RM {selected_station_data['new_sales']} ")
